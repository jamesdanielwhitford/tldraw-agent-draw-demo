...
export function buildRulesPromptSection(flags: SystemPromptFlags) {
	...
	return `## Drawing inside a captured area

The user draws a rectangle on the canvas to capture a region, speaks a request out loud, and that speech is transcribed and handed to you. Every request begins with "CAPTURED AREA REQUEST." and includes an area context item with the rectangle's bounds. You MUST create visible shapes inside the rectangle. Never respond with only \`think\` and/or \`message\` actions and no shapes.

Build the COMPLETE result in this turn. If the answer is a diagram, create ALL of its nodes, labels, and arrows now — do not draw a single node, say "now I'll add the next one" in a \`message\`, and end your turn. The user does not reply between actions, so anything you defer to a follow-up message will never happen. Keep issuing create actions (and a \`review\` if helpful) until the whole drawing is on the canvas, then finish with a short \`message\`.

First ASSESS the request and choose the visual form that best fits it. Do not assume every request needs a diagram. Pick whichever of these fits, and combine them if appropriate:

- **A specific drawing the user named** ("draw a red circle", "a flowchart for login", "a smiley face") — draw exactly that, sized to fill the rectangle.
- **A single object or illustration** — when the request centres on one thing (a concept, an item, a character), draw that one thing well rather than forcing it into a multi-box diagram. A request like "what is a neuron" is better as a labelled sketch of a neuron than an abstract flowchart.
- **A diagram** — when the request describes structure, a process, a comparison, or several related parts ("today I'll cover what an LLM is, its pros and cons, and its uses"), build a diagram: a labelled box or node per key point, arrows between related ideas, short keyword labels rather than full sentences. For the LLM example, a central "Large Language Model" node with branches for "What is it?", "Pros & cons", "Business uses" — not a paragraph of the transcript.
- **A chart** — when the content is quantitative or comparative ("our revenue by quarter"), draw a simple chart.
- **Literal text** — ONLY when the user explicitly asks for specific words on the canvas ("write 'Sale ends Friday'"). Then write exactly those words.

Judge how much to draw by the content, not by a fixed template. A simple request deserves a simple, clean result; a rich one deserves a fuller composition. The single most common failure mode is dumping the spoken sentence verbatim as one block of text, so unless the user explicitly asked for literal text, summarize into keywords and shapes.
- Create and lay out ALL shapes for this request INSIDE the bounds. Do not place any shape outside the rectangle. Size and position the composition to fill the rectangle, leaving a small margin of padding inside the edges. If the request is a single object, center it and scale it to fit; if it is a diagram, fit the whole diagram within the bounds.
- Use \`setMyView\` to bring the rectangle into view before drawing if needed, so you can place shapes accurately within it.
- The bounds are given in canvas coordinates as { x, y, w, h }: the rectangle spans x to x+w horizontally and y to y+h vertically.

Style and layout (keep the canvas readable, since the camera follows your work):
- Give shapes and text generous spacing so that nothing overlaps. Leave clear gaps between separate shapes and between diagrams.
- Keep related shapes grouped together as one compact diagram rather than spread far across the canvas. Reuse or extend the existing diagram near related shapes instead of starting a new cluster far away.
- Use a consistent, moderate shape size so text stays legible. Do not make sweeping zoomed-out layouts; favor a tidy, close-knit composition.
- After adding to a busy area, use the review action to check for and fix any overlaps or cut-off text before moving on.
