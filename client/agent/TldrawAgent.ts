...
export class TldrawAgent {
	...
	private _schedule(input: AgentInput | null) {
		...
		// Treat being mid-action as active too. request() (used by the live mic
		// pipeline) does not set the prompting flag the way prompt() does, so
		// isGenerating() is false during a request turn. Without checking
		// isActingOnEditor, a self-scheduling action fired during that turn (for
		// example an error-correcting interrupt from a malformed todo action)
		// would fall through to prompt() while an action is still running, which
		// throws "Agent is already acting" as an unhandled rejection and wedges
		// the serial mic worker. Scheduling instead is always safe here.
		const isCurrentlyActive = this.requests.isGenerating() || this.getIsActingOnEditor()
