...
/**
 * Normalize a label alignment to a value tldraw's geo/note `align` prop accepts
 * (`start` | `middle` | `end`). The model sometimes emits the CSS-style `center`
 * (or `left`/`right`), which would otherwise crash tldraw's schema validation.
 */
function asLabelAlign(value: unknown): 'start' | 'middle' | 'end' | undefined {
	switch (value) {
		case 'start':
		case 'middle':
		case 'end':
...
function convertGeoShapeToTldrawShape(
	...
	return {
		shape: {
			...
			props: {
				align: asLabelAlign(focusedShape.textAlign) ?? defaultGeoShape.props?.align ?? 'middle',
